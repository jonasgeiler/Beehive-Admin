<script>
	...
	function handleClick() {
		...
		transitionText = newActive ? 'Starting...' : 'Stopping...';

		bees.setActive(bee, newActive)
		    .then(() => {
			    changingState = true;

			    setTimeout(() => state = newActive, 300); // Fire when text gone
			    setTimeout(() => {
				    changingState = false;
				    transitionText = '';
			    }, 600); // Fire when ball at end position
		    })
		    .catch(() => transitionText = 'Error!');
	}
</script>
...
<div class="switch" class:big class:active={state} class:changing-state={changingState} on:click={handleClick}>
	...
	<div class="switch-text">
		<span class="switch-text-inner">{transitionText || (state ? 'Running' : 'Stopped')}</span>
	</div>
</div>
